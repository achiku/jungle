...
@pytest.mark.parametrize('inst_name, use_inst_id, username, keyfile, port, ssh_options, use_gateway, expected', [
    ('ssh_server', False, 'ubuntu', 'key.pem', 22, "-o StrictHostKeyChecking=no", False,
        'ssh ubuntu@{} -i key.pem -p 22 -o StrictHostKeyChecking=no'),
    ('ssh_server', False, 'ubuntu', None, 22, None, False, 'ssh ubuntu@{} -p 22'),
    (None, True, 'ubuntu', 'key.pem', 22, None, False, 'ssh ubuntu@{} -i key.pem -p 22'),
    ('ssh_server', False, 'ubuntu', 'key.pem', 22, None, True, 'ssh -tt ubuntu@{} -i key.pem -p 22 ssh ubuntu@{}'),
    ('ssh_server', False, 'ubuntu', None, 22, None, True, 'ssh -tt ubuntu@{} -p 22 ssh ubuntu@{}'),
    (None, True, 'ubuntu', 'key.pem', 22, None, True, 'ssh -tt ubuntu@{} -i key.pem -p 22 ssh ubuntu@{}'),
    (None, True, 'ubuntu', None, 22, "-A", True, 'ssh -tt ubuntu@{} -p 22 -A ssh ubuntu@{}'),
])
def test_create_ssh_command(
        mocker, ec2, inst_name, use_inst_id, username, keyfile, port, ssh_options, use_gateway, expected):
    ...
    ssh_command = create_ssh_command(
        ssh_server_instance_id, inst_name, username, keyfile, port, ssh_options, gateway_server_instance_id)
    if use_gateway:
