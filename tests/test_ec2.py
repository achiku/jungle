...
@pytest.mark.parametrize('inst_name, use_inst_id, username, keyfile, keypath, port, use_gateway, expected', [
...
def test_create_ssh_command(mocker, ec2, inst_name, use_inst_id, username, keyfile, keypath, port, use_gateway, expected):
    ...
    ssh_command = create_ssh_command(
        ssh_server_instance_id, inst_name, username, keyfile, keypath, port, gateway_server_instance_id)
    if use_gateway:
